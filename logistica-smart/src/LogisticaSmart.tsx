 ...
 * Tema: Dark Mode "Grafite & Verde-Menta"
 *
 ...
 * 1. Clustering por prefixo de CEP (3 dígitos = sub-região dos Correios).
 *    → Alternativa a K-Means: determinística, explicável ao usuário e
 *      O(n), suficiente para o volume de um e-commerce pequeno/médio.
 * 2. Rota via heurística do Vizinho Mais Próximo (Nearest Neighbor).
 *    → O problema real (TSP) é NP-difícil; a heurística entrega uma
 *      rota ~25% acima do ótimo em O(n²), o que é um trade-off honesto
 *      e defensável para uso operacional.
 * 3. Distância pela fórmula de Haversine (esfera terrestre), não
 *    distância euclidiana — coordenadas geográficas exigem isso.
 * 4. Toda a lógica de negócio é pura (sem React) → testável com Vitest
 *    sem renderizar nada.
 */
